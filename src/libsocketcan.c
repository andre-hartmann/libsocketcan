...
{
	// Keep currently set data bitrate
	struct can_bittiming dataBitTiming;
	int result = can_get_data_bittiming(name, &dataBitTiming);
	if (result == -1)
		return result;
	...
	if (dataBitTiming.bitrate) {
		dataBitTiming.bitrate = 0;
...
{
	// Keep currently set arbitration bitrate
	struct can_bittiming bitTiming;
	int result = can_get_bittiming(name, &bitTiming);
	if (result == -1)
		return result;
